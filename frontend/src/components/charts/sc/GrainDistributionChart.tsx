...
const GrainDistributionChart: React.FC<GrainDistributionChartProps> = ({ psd, sieveData, height = 350 }) => {
  ...
  return (
    <ChartContainer title={t('sandControl.charts.grainCurveTitle')} icon={BarChart3} height={height}
      ...
      <LineChart data={data} margin={CHART_DEFAULTS.margin}>
        ...
        <XAxis dataKey="size" stroke={CHART_DEFAULTS.axisColor} tick={{ fill: CHART_DEFAULTS.axisColor, fontSize: 11 }}
          label={{ value: t('sandControl.charts.grainSizeAxis'), position: 'bottom', fill: CHART_DEFAULTS.axisColor }}
          reversed />
        <YAxis stroke={CHART_DEFAULTS.axisColor} tick={{ fill: CHART_DEFAULTS.axisColor, fontSize: 12 }}
          domain={[0, 100]}
          label={{ value: t('sandControl.charts.passingPct'), angle: -90, position: 'insideLeft', fill: CHART_DEFAULTS.axisColor }} />
        ...
        <Line type="monotone" dataKey="passing" stroke="#f59e0b" strokeWidth={2} dot={{ fill: '#f59e0b', r: 4 }} name={t('sandControl.charts.passingPct')} />
      </LineChart>
    </ChartContainer>
  );
};
