...
class TorqueDragEngine:
    ...
    def compute_torque_drag(
        ...
        for i in range(len(survey) - 1, 0, -1):
            ...
            # When pipe is buckled, it contacts the wellbore wall continuously,
            # generating additional lateral force and therefore extra drag.
            f_drag_extra = 0.0
            torque_extra = 0.0
            if buckling != "OK" and fa < 0:
                ...
                ei_local = TorqueDragEngine.STEEL_E * i_mom  # lb-in^2
                ...
                # Buoyed weight per inch for this section
                w_per_in = ds["weight"] * bf / 12.0
                compression = abs(fa)

                if buckling == "SINUSOIDAL":
                    # Mitchell: F_drag_extra = mu * F^2 / (4 * w_n * r)
                    ...
                    # Lubinski: F_drag_extra = mu * r * F^2 / (2 * EI)
                    if ei_local > 0:
                        f_drag_extra = mu * r_clear * compression**2 / (2.0 * ei_local) * delta_md * 12.0

                # Post-buckling torque (continuous wall contact)
                if operation in ("rotating", "back_ream"):
                    r_contact_in = od / 2.0  # inches
                    torque_extra = mu * r_clear * compression * r_contact_in / 12.0  # ft-lb
